...
class MultiTaskLossWrapper(nn.Module):
    ...
    def forward(self, data, AUs, AU_intensities, device):
        ...
        loss_collect = 1/((self.log_sigmas[0]**2)) * AU_loss + rel(torch.log(self.log_sigmas[0])/self.task_num)
        ...
        for i, lab in enumerate(AU_intensities.permute(1,0)):
            ...
            if len(AU_idx) > 0:
                ...
                loss_collect += 1/((self.log_sigmas[i]**2)) * au_tmp_loss + rel(torch.log(self.log_sigmas[i])/self.task_num)
