...
class MultiTaskLossWrapper(nn.Module):
    ...
    def forward(self, data, AUs, AU_intensities, device):
        ...
        loss_collect = 0.5 * torch.exp(-2*self.log_sigmas[0]) * AU_loss + rel(self.log_sigmas[0])
        #loss_collect = (2 * (self.log_sigmas[0]**2))**(-1) * AU_loss + rel(torch.log(self.log_sigmas[0]))
        ...
        for i, lab in enumerate(AU_intensities.permute(1,0)):
            ...
            if len(AU_idx) > 0:
                ...
                loss_collect += 0.5 * torch.exp(-2*self.log_sigmas[i])*au_tmp_loss + rel(self.log_sigmas[i])
                #loss_collect += (2 * (self.log_sigmas[i]**2))**(-1) * au_tmp_loss + rel(torch.log(self.log_sigmas[i]))
