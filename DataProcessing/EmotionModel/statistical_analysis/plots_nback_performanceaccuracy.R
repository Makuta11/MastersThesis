...
# spagetti plot
p1 <- ggplot(data = d1_4obs_stim0, aes(x = Task, y = Score, group = ID, colour=Period)) +
  mytheme + 
  coord_trans(ylim = c(70,100))  +  
  geom_line(size=1.5) + 
  ggtitle("Continuous") +
  scale_color_manual(values=c("#007ea7", "#00a8e8")) +
  labs(y = "Performance Score %", x = "Stimulation") + 
  scale_x_discrete(limits = c("Pre","Post"), expand = c(0.1, 0.1))

p2 <- ggplot(data = d1_4obs_stim1, aes(x = Task, y = Score, group = ID, colour=Period)) +
  mytheme + 
  coord_trans(ylim = c(70,100))  +  
  geom_line(size=1.5) + 
  ggtitle("ISF") +
