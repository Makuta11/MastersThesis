...
# Set up 
m <- glmer(Scores ~ Stim*Session + (1 | ID), data=d3, family="binomial")
summary(m)
...
stim0_pre <- d3_stim0$Scores[d3_stim0$Task == "Pre"]
...
# Side
p1 <- ggplot(data = d3_stim0, aes(x = Scores, fill = Task)) +
  mytheme + 
  ggtitle("Continuous") +
  geom_density(adjust=1, alpha=.4, aes(x=Scores, y = -..density..), subset(d3_stim0, Task == "Pre") ) +
  geom_density(adjust=1, alpha=.4, aes(x=Scores, y = ..density..), subset(d3_stim0, Task == "Post") ) +
...
p2 <- ggplot(data = d3_stim1, aes(x = Scores, fill = Task)) +
  mytheme + 
  ggtitle("ISF") +
  labs(x = "Stimulation") + 
  geom_density(adjust=1, alpha=.4, aes(x=Scores, y = -..density..), subset(d3_stim1, Task == "Pre") ) +
  geom_density(adjust=1, alpha=.4, aes(x=Scores, y = ..density..), subset(d3_stim1, Task == "Post") ) +
  coord_flip(ylim = c(-4,4)) + 
