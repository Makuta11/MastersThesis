...
mydata$Stim <- factor(ifelse(mydata$Stim == 1,"ISF","Continuous"), levels = c( "Continuous","ISF"))
...
### Performance Score ###
#m <- lmer(Score ~ Stim*Period + (1|ID), data=d1)
m <- lme(Score ~  Stim*Period , data=d3, random = ~ 1 |ID,  method="ML")
print(m)
plot(m)
summary(m)
anova(m)
...
### Response Time ###
# Set up 
#m <- lmer(Score ~ Stim*Period + (1|ID), data=d3, family = poisson)
m <- lme(ResponsTime ~  Stim*Period , data=d2, random = ~ 1 |ID, method="ML")
print(m)
plot(m)
summary(m)
anova(m)
...
# spagetti plot
p <- ggplot(data = d3, aes(x = Stim, y = ResponsTime, fill=Stim)) +
  mytheme + 
  geom_violin(trim=FALSE) + 
  geom_boxplot(width=0.1, fill="white") +
  ggtitle("Continuous") +
